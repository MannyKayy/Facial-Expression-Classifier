...
def AffineBackward(grad_y, x, w):
    ...
    N = x.shape[0]
    grad_x = grad_y.dot(np.transpose(w)).reshape(x.shape)
    grad_w = (np.transpose(x.reshape(N, np.prod(x.shape[1:])))).dot(grad_y)
...
def ReLUBackward(grad_y, x, y):
    ...
    Args:
        grad_y: Gradient from last layer.
        x: Inputs. dh1
        y: Output of ReLU using input x. dh2
...
def NNUpdate(model, eps, momentum):
    ...
    # Insert your code here.
    # Update the weights.
    tempW1 = model['W1'] - model['vW1']
    tempW2 = model['W2'] - model['vW2']
    tempW3 = model['W3'] - model['vW3']
    tempb1 = model['b1'] - model['vb1']
    tempb2 = model['b2'] - model['vb2']
    tempb3 = model['b3'] - model['vb3']

    tempvW1 = np.dot(momentum, model['vW1']) + np.dot(eps, model['dE_dW1'])
    tempvW2 = np.dot(momentum, model['vW2']) + np.dot(eps, model['dE_dW2'])
    tempvW3 = np.dot(momentum, model['vW3']) + np.dot(eps, model['dE_dW3'])
    tempvb1 = np.dot(momentum, model['vb1']) + np.dot(eps, model['dE_db1'])
    tempvb2 = np.dot(momentum, model['vb2']) + np.dot(eps, model['dE_db2'])
    tempvb3 = np.dot(momentum, model['vb3']) + np.dot(eps, model['dE_db3'])
