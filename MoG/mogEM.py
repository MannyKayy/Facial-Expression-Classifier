...
def q2():
    ...
    xside = np.arange(1, (iters+1))

    print(p)
    #print(logLikelihood)
    plt.figure(2)
    plt.plot(xside, logLikelihood)
    plt.xlabel('Iteration')
    plt.ylabel('Log Likelihood')
    plt.show()

    plt.figure(3)
    plt.plot([1,2,3,4,5,6,7], p)
    plt.xlabel('Number of Clusters in the Gaussian Mixture')
    plt.ylabel('Propbabilities of Clusters (Mixing Coefficients)')
    plt.title("Probabilities of Clusters")
    plt.show()
...
def q4():
    ...
    for t in xrange(T):
        ...
        #-------------------- Add your code here ------------------------------
        pAng, muAng, varyAng, logAng = mogEM(x_train_anger, K, iters, randConst, minVary)
        pHap, muHap, varyHap, logHap = mogEM(x_train_happy, K, iters, randConst, minVary)
        #------------------- Answers ---------------------
        ...
        #-------------------- Add your code here ------------------------------
        #mogLogLikelihood(p, mu, vary, x)
        trainProbAng = mogLogLikelihood(pAng, muAng, varyAng, x_train)
        trainProbHap = mogLogLikelihood(pHap, muHap, varyHap, x_train)
        errorTrain[t] = np.mean((trainProbAng > trainProbHap) == y_train)
        
        validProbAng = mogLogLikelihood(pAng, muAng, varyAng, x_valid)
        validProbHap = mogLogLikelihood(pHap, muHap, varyHap, x_valid)
        errorValidation[t] = np.mean((validProbAng > validProbHap) == y_valid)
        
        testProbAng = mogLogLikelihood(pAng, muAng, varyAng, x_test)
        testProbHap = mogLogLikelihood(pHap, muHap, varyHap, x_test)
        errorTest[t] = np.mean((testProbAng > testProbHap) == y_test)
        #------------------- Answers ---------------------
    ...
    plt.title('Change in Error with Number of Mixture Components')
    plt.legend()
    plt.draw()
